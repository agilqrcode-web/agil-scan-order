...
export default async function handler(req, res) {
  ...
  if (req.url.includes('/check-profile')) {
    if (req.method !== 'POST') { // check-profile was POST, onboard-user was POST
  ...
  if (req.url.includes('/onboard-user')) {
    if (req.method !== 'POST') {
      return res.status(405).json({ error: 'Method Not Allowed' });
    }
  ...
  // Webhook will always be POST
  if (req.method === 'POST') {
    // Check if it's a Svix webhook by checking headers
