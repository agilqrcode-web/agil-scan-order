        ...
        <div className="pb-28"> {/* Padding for the fixed bottom bar */}
                            ...
                            <>
                                                ...
                                                <div className="flex-1 flex flex-col">
                                                    ...
                                                    <div className="flex justify-between items-center mt-2">
                                                        ...
                                                        <div className="flex items-center">
                                                            ...
                                                            <Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:bg-red-100 hover:text-red-600 ml-2" onClick={() => removeFromCart(item.id)}><Trash2 className="h-4 w-4" /></Button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </li>
                                ...
                                <div className="mt-6 space-y-2 border-t pt-6">
                                    ...
                                    <dl className="space-y-1 text-sm text-gray-600 pt-4">
                                        <div className="flex justify-between"><dt>Sub-total</dt><dd>R$ {totalPrice.toFixed(2).replace('.', ',')}</dd></div>
                                        <div className="flex justify-between"><dt>Taxa de Serviço (10%)</dt><dd>R$ {serviceFee.toFixed(2).replace('.', ',')}</dd></div>
                                        <div className="flex justify-between font-bold text-base text-gray-800 border-t pt-2 mt-2"><dt>Total</dt><dd>R$ {finalTotal.toFixed(2).replace('.', ',')}</dd></div>
                                    </dl>
                                </div>
                            </>
                        )}
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50">
                <div className="container mx-auto max-w-3xl h-20 flex items-center justify-end px-4 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] border-t rounded-t-lg">
                    {activeOrderId && (
                        <Button size="lg" variant="outline" className="mr-4" onClick={() => navigate(`/order-status/${activeOrderId}`)}>
