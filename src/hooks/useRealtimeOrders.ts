...
export function useRealtimeOrders() {
  const { realtimeChannel } = useSupabase(); // MODIFIED DESTRUCTURING
  const queryClient = useQueryClient();
  // Removed isSignedIn usage

  useEffect(() => {
    console.log('[RT-DEBUG] useEffect START. Realtime Channel available:', !!realtimeChannel); // MODIFIED LOG
    if (!realtimeChannel) { // MODIFIED GUARD
      console.log('[RT-DEBUG] Bailing out: Realtime Channel not ready.'); // MODIFIED LOG
      return;
    }
